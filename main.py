...
C_CANCEL_REASON, D_CANCEL_REASON = 16, 17  # Added for Cancel Reasons
...
async def customer_cancel_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    b_id = query.data.split("_")[1]
    context.user_data['cancel_b_id'] = b_id
    await query.edit_message_text(f"{query.message.text}\n\n❓ Please type your reason for cancellation below:")
    return C_CANCEL_REASON

async def customer_cancel_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    reason = update.message.text
    b_id = context.user_data.get('cancel_b_id')
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not booking or booking.status in ["CANCELLED", "TRIP_COMPLETED"]:
            await update.message.reply_text("❌ Cannot cancel this booking or it is already cancelled.")
            context.user_data.pop('cancel_b_id', None)
            return ConversationHandler.END
        ...
        if booking.driver_id:
            ...
            if driver:
                ...
                await context.bot.send_message(
                    ...
                    text=f"⚠️ Customer cancelled Job #{b_id}.\nReason: {reason}\nYour {points_refund:,.0f} points have been automatically refunded."
                )

        booking.status = "CANCELLED"
        await session.commit()
        await update.message.reply_text(f"🚫 **BOOKING CANCELLED**\nJob: #{b_id}\nReason: {reason}")
    
    context.user_data.pop('cancel_b_id', None)
    return ConversationHandler.END


# --- Driver Cancellation Request Flow ---
async def driver_cancel_prompt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    b_id = query.data.split("_")[1]
    context.user_data['d_cancel_b_id'] = b_id
    await query.edit_message_text(f"{query.message.text}\n\n❓ Please type your reason for cancelling this job below:")
    return D_CANCEL_REASON

async def driver_cancel_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    reason = update.message.text
        ...
        if booking:
            try:
                await context.bot.send_message(
                    ...
                    text=f"⚠️ **Notice:** Your driver has requested to cancel the trip.\n**Reason:** {reason}\n\nPlease wait while we process this or find a new driver."
                )
            except Exception as e:
                logger.error(f"Failed to send passenger cancel notice: {e}")
    ...
    if ADMIN_GROUP_ID:
        ...
        await update.message.reply_text(f"⏳ Cancellation requested.\nReason: {reason}\nWaiting for admin approval.")
    else:
    ...
    if data.startswith("approve_driver_"): 
        ...
        async with AsyncSessionLocal() as session:
            ...
            if booking and booking.status not in ["CANCELLED", "TRIP_COMPLETED"]:
                ...
                await context.bot.send_message(chat_id=booking.customer_id, text=f"⚠️ We have approved the driver's cancellation for Job #{b_id}. The job is back on dispatch to find a new driver.")
...
async def startup_event(): 
    ...
    customer_cancel_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(customer_cancel_prompt, pattern="^ccancel_")],
        states={
            C_CANCEL_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, customer_cancel_reason)]
        },
        fallbacks=[CommandHandler("start", start)]
    )

    driver_cancel_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(driver_cancel_prompt, pattern="^dcancelreq_")],
        states={
            D_CANCEL_REASON: [MessageHandler(filters.TEXT & ~filters.COMMAND, driver_cancel_reason)]
        },
        fallbacks=[CommandHandler("start", start)]
    )
    ...
    telegram_app.add_handler(customer_cancel_conv)
    telegram_app.add_handler(driver_cancel_conv)
