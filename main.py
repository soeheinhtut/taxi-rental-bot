...
OSRM_TIMEOUT_SECONDS = float(
    os.getenv("OSRM_TIMEOUT_SECONDS", "8")
)
...
TOPUP_PACKAGES = {
    "pkg_1": {
        "points": 1,
        "price": 1 * MMK_PER_POINT,
    },
    "pkg_10": {
        "points": 10,
        "price": 10 * MMK_PER_POINT,
    },
    "pkg_50": {
        "points": 50,
        "price": 50 * MMK_PER_POINT,
    },
    "pkg_100": {
        "points": 100,
        "price": 100 * MMK_PER_POINT,
    },
    "pkg_1000": {
        "points": 1000,
        "price": 1000 * MMK_PER_POINT,
    },
}
...
def normalize_picker_time(
    ...
    min_allowed = (
        ...
        else max(SERVICE_START_MINUTES, int(min_total))
    )

    min_allowed = (
        (min_allowed + TIME_STEP_MINUTES - 1)
        // TIME_STEP_MINUTES
    ) * TIME_STEP_MINUTES

    total_minutes = max(
        min_allowed,
        min(SERVICE_END_MINUTES, total_minutes),
    )

    total_minutes = (
        total_minutes // TIME_STEP_MINUTES
    ) * TIME_STEP_MINUTES
...
def next_quarter_hour(dt_obj, lead_minutes=0):
    ...
    minute_floor = (
        candidate.minute // TIME_STEP_MINUTES
    ) * TIME_STEP_MINUTES
    ...
    if (
        candidate.minute < dt_obj.minute
        or (
            candidate == dt_obj
            and lead_minutes > 0
        )
    ...
    if (
        lead_minutes > 0
        and candidate < dt_obj + timedelta(minutes=lead_minutes)
    ):
        candidate += timedelta(
            minutes=TIME_STEP_MINUTES
        )
...
def get_default_time_for_date(selected_date):
    ...
    minimum = next_quarter_hour(
        now,
        INSTANT_DISPATCH_LEAD_MINUTES,
    )
    ...
    normalized = normalize_picker_time(
        minimum.hour,
        minimum.minute,
        minimum_total,
    )
...
def get_time_picker_keyboard(
    ...
    hour24, minute = normalize_picker_time(
        hour24,
        minute,
        min_total,
    )

    current_time = format_picker_time(
        hour24,
        minute,
    )
    ...
    prev_total = max(
        min_allowed,
        current_total - TIME_STEP_MINUTES,
    )
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                ...
                callback_data=(
                    f"time_hour_"
                    f"{min(SERVICE_END_MINUTES // 60, hour24 + 1)}"
                ),
            ),
            InlineKeyboardButton(
                "⬆️ +15 Min",
                callback_data=(
                    f"time_total_{next_total}"
                ),
            ),
        ],
        [
        ...
        [
            InlineKeyboardButton(
                "⬇️ -1 Hour",
                callback_data=(
                    f"time_hour_{max(5, hour24 - 1)}"
                ),
            ),
            InlineKeyboardButton(
                "⬇️ -15 Min",
                callback_data=(
                    f"time_total_{prev_total}"
                ),
            ),
        ],
        [
            InlineKeyboardButton(
                "ℹ️ 05:00 AM – 10:00 PM • 15-min steps",
                callback_data="time_noop",
            ),
        ],
    ]
...
def get_calendar_keyboard(year, month):
    ...
    if shown_month_start > current_month_start:
        prev_button = InlineKeyboardButton(
            "⏪",
            callback_data=(
                f"cal_{prev_year}_{prev_month}"
            ),
        )
    else:
        prev_button = InlineKeyboardButton(
    ...
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "⚡ Book Now (ASAP)",
                    callback_data="mode_instant",
                )
            ],
            [
                InlineKeyboardButton(
                    "📅 Schedule for Later",
                    callback_data="mode_scheduled",
                )
            ],
        ]
    )
...
async def get_road_route_km_duration(
    pickup_lat,
    pickup_lng,
    drop_lat,
    drop_lng,
    ...
    try:
        ...
        straight_km = calculate_distance(
            pickup_lat,
            pickup_lng,
            drop_lat,
            drop_lng,
...
async def start(
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "💰 Driver Profile & Check Balance",
                callback_data="driver_balance",
            )
        ],
    ]
...
async def check_balance_callback(
    ...
    async with AsyncSessionLocal() as session:
        ...
        text = (
            "👤 **Driver Profile & Wallet Status**\n\n"
            f"📛 Name: {driver.name}\n"
            f"💰 Point Balance: "
            f"**{driver.wallet_balance:,.0f} Points**\n"
...
async def start_booking_callback(
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                "🚕 Kilo Car (Min 5km 8,500 MMK)",
                callback_data="Kilo Car",
            )
        ],
        [
            InlineKeyboardButton(
...
async def booking_mode_chosen(
    ...
    if mode == "mode_instant":
        ...
        context.user_data["time"] = (
            pickup_dt.strftime("%I:%M %p")
        )

        context.user_data["time_hour"] = (
            pickup_dt.hour
        )

        context.user_data["time_minute"] = (
            pickup_dt.minute
        )

        context.user_data["time_min_total"] = (
            SERVICE_START_MINUTES
        )

        return await proceed_after_time_selection(
            query,
            context,
            instant=True,
        )
    ...
    if mode == "mode_scheduled":
        ...
        context.user_data["booking_mode"] = (
            "SCHEDULED"
        )

        await query.edit_message_text(
            f"🚘 Vehicle: "
            f"**{context.user_data.get('vehicle', 'Sedan')}**\n\n"
            "📅 Select Pickup Date:",
...
async def proceed_after_time_selection(
    query,
    context,
    instant=False,
):
    ...
    if vehicle in ["TAXI", "Kilo Car"]:
        ...
        pickup_prompt = (
            "📍 Please share your exact "
            "**GPS Pickup Location**."
            if vehicle == "Kilo Car"
            else
            "📍 Please share your exact "
            "GPS Pickup Location or "
            "type your address:"
        )
    ...
    keyboard = [
        [
            InlineKeyboardButton(
                f"3 Hours ({packages[3]:,.0f} MMK)",
                callback_data="3",
            )
        ],
        [
            InlineKeyboardButton(
                f"5 Hours ({packages[5]:,.0f} MMK)",
                callback_data="5",
            )
        ],
        [
            InlineKeyboardButton(
                f"8 Hours ({packages[8]:,.0f} MMK)",
                callback_data="8",
            )
        ],
    ]
...
async def date_chosen(
    ...
    if data.startswith("date_"):
        ...
        hour24, minute, min_total = (
            get_default_time_for_date(
                selected_date
            )
        )
        ...
        await query.edit_message_text(
            ...
            "🕐 **Select Pickup Time**\n\n"
            "Use the scroll buttons to choose "
            "a time in 15-minute steps:",
            reply_markup=reply_markup,
            parse_mode="Markdown",
        )
...
async def time_chosen(
    ...
    if data == "time_confirm":
        ...
        if (
            selected_total < min_total
            or selected_total > SERVICE_END_MINUTES
        ):
            await query.answer(
                "❌ Please choose a valid service time.",
                show_alert=True,
            )
            return TIME

        selected_time = format_picker_time(
            hour24,
            minute,
        )
...
async def hours_chosen(
    ...
    await query.edit_message_text(
        ...
        f"💰 **Total Fare:** "
        f"{total_fare:,.0f} MMK\n\n"
        "📍 Please share your exact GPS "
        "pickup location or type your address:",
        parse_mode="Markdown",
    )
...
async def location_received(
    ...
    if context.user_data.get(
        ...
        if (
            ...
            await update.message.reply_text(
                "❌ Kilo Car requires GPS "
                "pickup location.\n\n"
                "Please tap the 📎 / Location "
...
async def drop_location_received(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
    ...
    if loc:
        ...
        context.user_data["drop_location"] = (
            f"https://maps.google.com/?q="
            f"{loc.latitude},{loc.longitude}"
        )

        context.user_data["drop_lat"] = (
            loc.latitude
        )

        context.user_data["drop_lng"] = (
            loc.longitude
        )
        ...
        await update.message.reply_text(
            ...
            "Please use 📎 (paperclip) "
            "→ Location to choose on the map."
        )
...
async def customer_phone_received(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
    ...
    data["preview_customer_id"] = (
        update.message.from_user.id
    )

    booking_id = (
        f"RNT-"
        f"{get_yangon_now().strftime('%Y%m%d')}-"
        f"{int(get_yangon_now().timestamp()) % 10000}"
    )
    ...
    if vehicle in ["TAXI", "Kilo Car"]:
        ...
        (
            ...
            route_minutes,
            distance_source,
        ) = await get_road_route_km_duration(
            p_lat,
            p_lng,
            d_lat,
            d_lng,
        ...
        calculated_fare = calculate_kilo_fare(
            road_km
        )
        ...
        final_location = (
            ...
            f"{data['location']}\n"
            f"**Drop-off:** "
            f"{data.get('drop_location', 'N/A')}\n"
            f"**🗺️ Route Map:** "
            f"{direction_link}"
        )

        hours_label = (
            "Point-to-Point (Kilo Car)"
        )
    ...
    if vehicle == "Kilo Car":
        ...
        end_label = (
            f"Scheduled End: "
            f"**{data.get('end_time', format_end_time(start_dt, hours_db_value * 60))}**"
        )
    ...
    if vehicle == "Kilo Car":
        ...
        summary += (
            f"\n📏 Road Distance: "
            f"**{data.get('route_distance_km', 0):.1f} km**\n"
            f"🛣️ Distance Basis: "
            f"{data.get('distance_source', 'N/A')}\n"
        )
        ...
        if data.get(
            ...
            summary += (
                f"⏱ Estimated Drive: "
                f"**{round(data['route_duration_minutes'])} min**\n"
            )
    ...
    if vehicle != "Kilo Car":
        ...
        summary += (
            "ℹ️ Please confirm that the "
            "pickup time, package, and "
            "scheduled end time are correct.\n\n"
        )
    ...
    data["pending_final_location"] = (
        final_location
    )

    data["pending_hours_label"] = (
        hours_label
    )

    data["pending_points_required"] = (
        points_required
    )

    data["pending_fare_display"] = (
        fare_display
    )

    data["pending_fare_db_value"] = (
        fare_db_value
    )

    data["pending_hours_db_value"] = (
        hours_db_value
    )
...
async def booking_confirmed(
    ...
    final_location = data[
        "pending_final_location"
    ]

    fare_display = data[
        "pending_fare_display"
    ]
    ...
    if (
        ...
        await query.edit_message_text(
            "⚠️ This pickup time has already passed. "
            "Please start a new booking and "
    ...
    booking = Booking(
        ...
        route_distance_km=data.get(
            "route_distance_km"
        ),
        route_duration_minutes=data.get(
            "route_duration_minutes"
        ),
        distance_source=data.get(
            "distance_source"
        ),
    )
    ...
    # Customer cancellation button is attached
    ...
    if DRIVER_GROUP_ID:
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=(
                    "⚠️ Booking was created, "
                    "but we could not notify "
                    "the driver group. "
...
# ============================================================
# CANCEL BEFORE BOOKING CONFIRMATION
# ============================================================

async def booking_cancelled_before_confirm(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
    ...
    await query.edit_message_text(
        "🚫 **Booking cancelled.**\n\n"
        "Use /start to make a new booking.",
        parse_mode="Markdown",
    )
...
async def customer_cancel_prompt(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
    ...
    # <== Update Phase1_21Sep26
    # Validate callback format before processing.
    if not query.data.startswith("ccancel_"):
        return ConversationHandler.END

    b_id = query.data.replace(
        "ccancel_",
        "",
        1,
    ).strip()
    ...
    # <== Update Phase1_21Sep26
    # Store cancellation request separately.
    context.user_data["cancel_b_id"] = b_id
    context.user_data["cancel_requested_by"] = (
        query.from_user.id
    )
    ...
    # <== Update Phase1_21Sep26
    # Explicitly return cancellation state.
    return C_CANCEL_REASON


async def customer_cancel_reason(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:

    # <== Update Phase1_21Sep26
    # Make sure this message belongs to
    # an active customer cancellation.
    ...
    if not b_id or not requested_by:
        ...
        await update.message.reply_text(
            ...
            "Please use /start to make a new booking."
        )
    ...
    async with AsyncSessionLocal() as session:
        ...
        # <== Update Phase1_21Sep26
        # Only the actual customer can cancel.
        if (
            booking.customer_id
            != update.effective_user.id
        ):
        ...
        # <== Update Phase1_21Sep26
        # Refund driver commission if driver was assigned.
            ...
            if driver:
                ...
                try:
                    ...
                    await context.bot.send_message(
                        ...
                        text=(
                            f"⚠️ Customer cancelled "
                            f"Job #{b_id}.\n\n"
                            f"Reason: {reason}\n\n"
                            f"💰 Your "
                            f"{points_refund:,.0f} "
                            f"points have been "
                            f"automatically refunded."
                        ),
                    )
                    ...
                    logger.error(
                        ...
                        f"about customer cancellation: {e}"
                    )
    ...
    # <== Update Phase1_21Sep26
    # Clear cancellation session only.
    context.user_data.pop(
        "cancel_b_id",
        None,
    )
...
async def driver_cancel_prompt(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
    ...
    # <== Update Phase1_21Sep26
    # Store driver cancellation session.
    context.user_data["d_cancel_b_id"] = b_id
...
async def driver_cancel_reason(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
...
async def driver_register_start(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
...
async def driver_name_received(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
...
async def driver_phone_received(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
...
async def driver_vehicle_received(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
...
async def driver_plate_received(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
...
async def topup_start(
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton(
                "10 Points",
                callback_data="pkg_10",
            ),
            InlineKeyboardButton(
                "50 Points",
                callback_data="pkg_50",
            ),
        ],
        [
            InlineKeyboardButton(
                "100 Points",
                callback_data="pkg_100",
            ),
            InlineKeyboardButton(
                "1,000 Points",
                callback_data="pkg_1000",
            ),
        ],
    ]
...
async def topup_package_chosen(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
...
async def topup_receipt_received(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
) -> int:
...
async def accept_job(
    ...
    async with AsyncSessionLocal() as session:
        ...
        if (
            ...
            await query.answer(
                "❌ You are not an approved driver!",
                show_alert=True,
            )
        ...
        for existing in existing_jobs:
            ...
            buffered_existing_start = (
                existing_start
                - timedelta(
                    minutes=DRIVER_SCHEDULE_BUFFER_MINUTES
                )
            )

            buffered_existing_end = (
                existing_end
                + timedelta(
                    minutes=DRIVER_SCHEDULE_BUFFER_MINUTES
                )
            ...
            if (
                ...
                conflict_text = (
                    ...
                    f"A "
                    f"{DRIVER_SCHEDULE_BUFFER_MINUTES}-minute "
                    "safety buffer is included."
                )
        ...
        fare_display = (
            f"{booking.fare_mmk:,.0f} MMK"
            if booking.fare_mmk > 0
            else "Kilo Car Rate"
        )
...
async def driver_location_handler(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
):
...
async def startup_event():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            CONFIRM_BOOKING: [
                ...
                CallbackQueryHandler(
                    booking_confirmed,
                    pattern="^booking_confirm_",
                ),

                CallbackQueryHandler(
                    booking_cancelled_before_confirm,
    ...
    customer_cancel_conv = ConversationHandler(

        # <== Update Phase1_21Sep26
        # Explicit entry point for customer
        # cancellation callback.
        entry_points=[
            CallbackQueryHandler(
                customer_cancel_prompt,
                pattern="^ccancel_[^_]+$",
            )
        ],
        ...
        # <== Update Phase1_21Sep26
        # Keep the cancellation conversation
        # active until the reason is submitted.
        allow_reentry=True,
    )
    ...
    driver_cancel_conv = ConversationHandler(

        # <== Update Phase1_21Sep26
        # Explicit driver cancellation entry.
        entry_points=[
            CallbackQueryHandler(
                driver_cancel_prompt,
                pattern="^dcancelreq_[^_]+$",
            )
        ],
    ...
    # <== Update Phase1_21Sep26
    # Cancellation conversations are registered
    # before generic callback handlers.
    telegram_app.add_handler(
        customer_cancel_conv
    )
